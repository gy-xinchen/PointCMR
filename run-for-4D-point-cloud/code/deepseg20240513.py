...
def deeplearningseg(model_path, test_dir, atlas_dir):
    with tf.Session() as sess:
        ...
        # Call and load neural network weights for segmentation models
        ...
        for data in sorted(os.listdir(test_dir)):
            ...
            # imagePreprocessing
            # 1 Process nifit files and save header file information
            # 2 Align images on a timeline
            # 3 Applied automatic contrast adjustment
            # 4 Used for detecting the end diastolic and systolic phases in cardiac phase detection
            # The above preprocessing operations have been encapsulated in the Docker image environment and cannot be explicitly called
            if not originalnii:
            ...
            for fr in ['ED', 'ES']:
                ...
                imageOrg = np.squeeze(image, axis=-1).astype(np.int16)  # remove single channel dim == [W,H,Z]
                tmp = imageOrg

                X, Y, Z = image.shape[:3]  # Ensure to take the first three dimensions
                ...
                for slice in range(Z):  # Obtain images for each layer
                ...
                # shutil.copyfile('{0}/test_mask/seg_lvsa_{1}.nii.gz'.format(data_dir, fr), '{0}/segs/seg_lvsa_{1}.nii.gz'.format(data_dir, fr))
