...
def mutual_information_loss(f_A, f_B, mine_network, ma_et=None):
    # 正样本：真实的 (f_A, f_B)
    joint = mine_network(f_A, f_B)

    # 负样本：打乱 f_B
    f_B_shuffle = f_B[torch.randperm(f_B.size(0))]
    marginal = mine_network(f_A, f_B_shuffle)
    ...
    if ma_et is not None:
        ma_et = 0.01 * torch.mean(joint).detach() + (1 - 0.01) * ma_et
    else:
        ma_et = torch.mean(joint).detach()

    # 返回负互信息作为损失（因为我们要最大化互信息）
    return loss, ma_et
...
def train_one_epoch(model, Model_3Dphase, mine_net, criterion, criterion_phase, optimizer, optimizer_phase, optimizer_mine, lr_scheduler, scheduler_phase, data_loader, device, epoch, print_freq):
    model.train()
    Model_3Dphase.train()
    ...
    for clip, target, phase, index, subject_index in metric_logger.log_every(data_loader, print_freq, header):
        columns = ["室间隔夹角", "心室质量指数"]
        text_features = get_text_features(subject_index, columns)
        ...
        output, PST_embedded, _ = model(clip, normalized_features)
        output_phase, CurveNet_embedded, _ = Model_3Dphase(phase)

        # flops, params = profile(model, (clip,))
        # print('flops: %.2f M, params: %.2f M' % (flops / 1e6, params / 1e6))

        mi_loss, _ = mutual_information_loss(PST_embedded, CurveNet_embedded, mine_net)
        ...
        loss = loss - mi_loss * args.Lambda_MINE
        loss_phase = criterion_phase(output_phase, smoothed_target)
        loss_phase = loss_phase - mi_loss * args.Lambda_MINE
    ...
    print(f"epoch_mi_loss: {epoch_mi_loss}")
...
def evaluate(model, Model_3Dphase, criterion, criterion_phase, data_loader, device, best_auc):
    ...
    with torch.no_grad():
        for clip, target, phase, video_idx, subject_index in metric_logger.log_every(data_loader, 100, header):
            columns = ["室间隔夹角", "心室质量指数"]
            ...
            output, _, _ = model(clip, normalized_features)
            output_phase, _, _ = Model_3Dphase(phase)
...
def main(args):
    ...
    # total_params_3dphase = count_parameters(CurveNetMINE(num_classes=2).to(device))
    # total_params_mine = count_parameters(MINE(128).to(device))
    #
    # # 计算总参数量
    # total_params = total_params_model + total_params_3dphase + total_params_mine
    # print(f"Total number of parameters (model + 3Dphase + MINE): {total_params}")
    ...
    optimizer_mine = torch.optim.Adam(mine_net.parameters(), lr=args.lr_MINE)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_loss = train_one_epoch(model, Model_3Dphase, mine_net, criterion, criterion_phase, optimizer, optimizer_phase, optimizer_mine, lr_scheduler, scheduler_phase, data_loader, device, epoch, args.print_freq)
        ...
        if auc_data > weight_best_auc:
            ...
            if args.output_dir:
                checkpoint = {
                    ...
                    'model_3Dphase': Model_3Dphase.state_dict(),
                    'mine_net': mine_net.state_dict(),
...
def parse_args():
    ...
    parser.add_argument('--lr_phase', type=float, default=0.001, metavar='LR', help='learning rate (default: 0.001, 0.1 if using sgd)')
    parser.add_argument('--lr_MINE', default=1e-4, type=float, help='initial learning rate')
    parser.add_argument('--Lambda_MINE', default=1e-3, type=float, help='initial learning rate')
    ...
    parser.add_argument('--output-dir', default=r'/root/autodl-tmp/dataword/PST_CurveNet1024_txt_MINE_ALL', type=str, help='path where to save')
    # resume
