# -*- coding: utf-8 -*-
# @Time    : 2024/12/2 16:19
# @Author  : yuan
# @File    : curvenet_util_copy.py
"""
@Author: Yue Wang
...
class LPFA(nn.Module):
    def __init__(self, in_channel, out_channel, k, mlp_num=2, initial=False):
        ...
        if not initial:
            self.xyz2feature = nn.Sequential(
                nn.Conv2d(18, in_channel, kernel_size=1, bias=False),
                nn.BatchNorm2d(in_channel))

        self.mlp = []
        for _ in range(mlp_num):
            if self.initial:
                self.mlp.append(nn.Sequential(nn.Conv2d(in_channel * 2, out_channel, 1, bias=False),
    ...
    def group_feature(self, x, xyz, idx):
        ...
        points = xyz.view(batch_size, num_points, 1, 6).expand(-1, -1, self.k, -1)  # bs, n, k, 3
...
class CurveNetMINE(nn.Module):
    def __init__(self, num_classes=2, k=20, setting='default'):
        super(CurveNetMINE, self).__init__()
        ...
        self.conv1 = nn.Linear(1024 * 2, 128, bias=False)
        self.conv2 = nn.Linear(128, num_classes)
    ...
    def forward(self, xyz):
        ...
        x_embedded = self.conv1(x)
        x = F.relu(self.bn1(x_embedded.unsqueeze(-1)), inplace=True).squeeze(-1)
        x = self.dp1(x)
        x = self.conv2(x)
        return x, x_embedded