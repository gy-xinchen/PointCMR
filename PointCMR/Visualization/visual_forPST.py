...
def test(args):
    ...
    with torch.no_grad():  # 在测试过程中不计算梯度
        for batch_idx, (clip, target, phase, video_idx, subject_index) in enumerate(data_loader_test):
            ...
            weighted_xyzs_map = torch.matmul(one_head_last_attn_score, xyzs_map) # 将位置与注意力权重加权 [2,320,320]*[2,320,3] == [2, 320, 3]
            reshaped_weighted_xyzs_map = weighted_xyzs_map.view(2, 10, 32, 3).cpu().numpy() # 这里得到的是重塑后的注意力加权矩阵 [2,10,32,3]

            # 这里需要将初始化conv高纬映射还原 [b, 10, 128, 3] --> [b, 20, 2048, 3]

            for sample in range(args.batch_size):
                for frame in range(10):  # 遍历时间维度 (假设时间维度是 [0, 1, ..., 9])
                    # 使用 frame 作为索引动态选择对应的帧数据
                    high_res_features = feature_propagation(
                        xyzs[:, frame, :, :],  # 根据当前帧选择 xyzs 数据 [2, 32, 3]
                        clip[:, frame, :, :],  # 根据当前帧选择 clip 数据 [2,1024,3]
                        torch.tensor(reshaped_weighted_xyzs_map[:, frame, :, :]).to(device)  # 根据当前帧选择加权 xyzs [2,32,3]
                    )

                    # high_res_features是将其恢复后的注意力加权矩阵 形状为 [1024, 3]
                    normalized_high_res_features = (high_res_features - high_res_features.min()) / (
                            high_res_features.max() - high_res_features.min())
                    normalized_high_res_features_sample = normalized_high_res_features[sample, :, :]

                    weighted_avg = normalized_high_res_features_sample.cpu().numpy()
                    weighted_avg = np.mean(weighted_avg, axis=1)
                    weighted_avg = (weighted_avg - weighted_avg.min()) / (weighted_avg.max() - weighted_avg.min())

                    # 提取当前时间步的点云数据 (形状为 [1024, 3])
                    points = clip[sample, frame, :, :].cpu().numpy()  # 使用 frame 获取当前时间步的点云数据
                    ...
                    # 保存点云为PLY文件
                    ply_filename = f"{save_dir}/point_cloud_{frame}.ply"
                    o3d.io.write_point_cloud(ply_filename, pcd)

            output_phase, _ = Model_3Dphase(phase)
